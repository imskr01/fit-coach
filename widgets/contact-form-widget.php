 ...
 if(isset($emailSent) && $emailSent == true) { ?>
             <div><p><?php _e( 'Thanks, your email was sent successfully.', 'fitcoach' ); ?></p></div>
				<?php } else { ?>
                        ...
                        <select name="contactClass" id="contactClass" placeholder="Class Name"> 
						<?php global $post; $args = array( 'post_type' => 'classes', 'numberposts' => -1 ); $posts = get_posts($args); foreach( $posts as $post ) : setup_postdata($post); ?>                		
                        <option value="<? echo $post->ID; ?>" placeholder="Class Name"><?php the_title(); ?></option> 
						<?php endforeach; ?> 
                        </select>
